...
patch_series:
  - version: v1
    ...
    date: 2026-08-02
    summary: "rseq 时间片扩展授予路径在开中断下调用要求关中断的 hrtimer_rearm_deferred_tif()，导致 hrtimer 锁反转硬死锁。原为单行 guard(irq)()。（08-02-002）"
...
merge_assessment:
  likelihood: high
  blocking_issues: ["reflow 改法需最终确认无其它开中断调用点"]
  next_action: "等待 PeterZ 对 v3 的最终 ack 后合入（critical 必修）。"
contribution_opportunities:
  - kind: testing
    description: "在 LOCKDEP + 高频 rseq TSE 授予负载下验证 reflow 版不再触发 hrtimer 锁反转，回帖 lockdep/lockup 验证数据（作者仍未附 runs）。"
generated_at: "2026-08-05T00:25:00"
source_email_count: 1
related_articles: ["sched-20260803-012-rseq-fix-hard-lockup-on-granted-time-slice-extension-v2", "sched-20260802-002-rseq-fix-hard-lockup-on-granted-time-slice-extension"]
