# KASAN: slab-use-after-free in owner_on_cpu via iava_remove (mutex optimistic spin) [iavf] [syzkaller]
...
---
subject: "KASAN: slab-use-after-free in owner_on_cpu via iava_remove (mutex optimistic spin) [iavf] [syzkaller]"
...
lore_url: "未获取到"
authors: [Yang Zi]
maintainers_involved: [Peter Zijlstra, Ingo Molnar, Waiman Long, Boqun Feng]
current_version: report
patch_series:
  - version: report
    msgid: "<20260809.owner_on_cpu.uaf@yangzi>"
    date: 2026-08-09
    summary: "同一 fuzzer 作者通过 3 个不同驱动路径（iavf、dw_edma_pcie、bna）复现同一 root cause：mutex 乐观自旋读取 owner 任务的 on_cpu 字段时，owner 任务结构体已被释放，触发 KASAN use-after-free。"
...
merge_assessment:
  likelihood: unknown
  blocking_issues: ["需要确认根因是 mutex 乐观自旋对 owner 任务 on_cpu 的访问缺少引用保护，还是调用方驱动问题"]
...
related_articles: []
tags: [sched/core, locking, crash, syzbot]
---
