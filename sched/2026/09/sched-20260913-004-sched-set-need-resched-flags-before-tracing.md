...
patch_series:
  - version: v1
    ...
    date: 2026-09-11
    summary: 'set_tsk_need_resched() 与 __resched_curr() 两处把 TIF_NEED_RESCHED（含本地分支的标志设置与远程分支的 set_nr_and_not_polling）提前到 sched_set_need_resched_tp 之前发出，消除 BPF tracepoint 经 rcu_read_unlock_special() 的递归栈溢出。'
    review_outcome: '09-13 kernel-patches CI AI 审查 bot 回帖：远程分支新顺序可能令 RV nrp monitor 在 any_thread_running 状态收到 schedule_entry_preempt 进入 INVALID_STATE（panic reactor 下 panic），建议远程分支单独保持先 tp 后标志或文档化乱序可接受；本地分支与 set_tsk_need_resched() 被确认无此问题。作者与人工维护者均未回应。'
merge_assessment:
  likelihood: unknown
  blocking_issues:
...
contribution_opportunities:
  - kind: testing
    description: '在 TIF_POLLING_NRFLAG 构型复现审查描述的 nrp monitor INVALID_STATE 场景，验证「tracepoint 事件乱序」是否实际发生'
  - kind: review
