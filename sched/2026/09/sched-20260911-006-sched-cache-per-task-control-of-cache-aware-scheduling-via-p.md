...
patch_series:
  - version: v1 (RFC)
    ...
    date: 2026-08-27
    summary: 'RFC 0/7：prctl 级 per-task 缓存分组控制，PR_SCHED_CACHE_SHARE_FROM 按 pid 对指定共享来源；patch 7 为文档。'
    review_outcome: '09-09 Shrikanth 提出四问；09-11 Tim 逐条回应（腾讯跨进程功能分组用例、零改动可用、运行时可分组、拒绝 cgroup 载体），Shrikanth 尚无跟进。'
