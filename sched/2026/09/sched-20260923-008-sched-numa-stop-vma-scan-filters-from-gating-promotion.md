...
## TL;DR
本文为增量更新，完整背景见 sched-20260919-009（v2 4 枚）。Gregory Price 发出 v3（7 枚）：把可回合的修复集扩为 5 枚（新增「scan PID-inactive VMA for promotion」），并补上 `Fixes:`/`Cc: stable` 标签（3/7 `Fixes: c574bbe91703`、4/7 `Fixes: fc137c0ddab2`），另加 2 枚独立的 mm 清理（BIT()、VMA flag helper）。cover 给出首个量化收益数据：DRAM 带宽 150-200→250+ GB/s、CXL 带宽 40-45→5-10 GB/s、请求延迟 >5ms→800us-2ms。

## 背景与问题
背景见 sched-20260919-009 与 sched-20260918-013：NUMA balancing 用 hinting fault 同时做任务放置与 memory-tier 晋升，几个为避免无效 socket-placement fault 而设的过滤器误伤了 promotion。生产部署（kernel.numa_balancing=2，768 GB DRAM + 256 GB CXL，两个 ~430 GB 数据库负载、大 shmem VMA）出现 NUMA balancing 行为劣化：修前 DRAM 带宽仅 150-200 GB/s、CXL 带宽 40-45 GB/s 打到设备饱和、请求延迟 >5 ms 且尾部长。

## 技术方案
系列核心思路不变（把 scan reasoning 塞进 prot_none 注入，同时保留 placement 过滤）。v3 结构变化：

- **patches 1-5 = 可回合的修复集**（须一起回退）：
  1. mm: support promotion-only NUMA hinting scans
  2. mm: allow shared folios to be promoted to a fast tier
  3. sched/numa: scan read-only file mappings in tiering mode（`Fixes: c574bbe91703`）
  4. sched/numa: separate VMA placement from scan continuation（`Fixes: fc137c0ddab2`；把 `vma_is_accessed()` 改名 `vma_needs_placement_scan()`，扫描续态移到 `task_numa_work()`）
  5. sched/numa: scan PID-inactive VMAs for promotion（新增）
- **patches 6-7 = 独立清理**：`mm: use BIT() for change_protection() flags`、`mm: use VMA flag helpers in NUMA balancing`。

## 版本演进与当前进展
- v2（09-11，4 枚）：见 sched-20260918-013/014。
...
## Maintainer 意见与讨论焦点
本日无维护者新表态（v3 首日）。此前 Zi Yan 的「测试范围」质询（见 sched-20260919-009）在 v3 cover 中以明确的生产部署背景与量化数据得到回应。无 NAK；3/4、4/4 的 VMA 判定实现意见（见前作）是否已全部落实，需待复审。
...
- lore（v2 cover，见前作）: https://lore.kernel.org/all/20260911001826.2109390-1-gourry@gourry.net/
