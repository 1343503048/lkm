...
authors: [Hyunwoo Kim, Tim Chen, Chen Yu]
maintainers_involved: [Tim Chen, Chen Yu, Peter Zijlstra]
current_version: v2
patch_series:
  - version: v1
    msgid: null
  ...
  - version: v2
    ...
    date: '2026-09-01'
    summary: '在 exec_mm_put_old() 前调用新增 sched_cache_exec_done() 做 synchronize_rcu()，覆盖所有关抢占的读者；3 文件 +17 行；Fixes df0d98475954 + Cc stable'
    review_outcome: 'Tim Chen 认为完整 grace period 落在 exec 路径不可接受，主张改用 sc_stat 与 mm 解耦 + refcount + call_rcu（prctl 系列前 2 片）；Chen Yu 确认解耦实现；Hyunwoo 已验证那 2 片可防住本 UAF；Peter Zijlstra 被点名后本日未回复'
...
merge_assessment:
  ...
  next_action: "Peter Zijlstra 或 Tim Chen 明确选定路线：直接收 v2 走 stable，或把 prctl 前 2 片拆成独立 fix 系列并带 Reported-by/Tested-by"
...
related_articles: [sched-20260831-009, sched-20260831-008]
tags:
- load_balance
- crash
- perf
