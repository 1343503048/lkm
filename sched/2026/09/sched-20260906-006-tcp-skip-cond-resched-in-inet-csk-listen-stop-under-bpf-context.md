...
- version: v2
  msgid: null
  date: 2026-09-06
  summary: bpf v2 2/3：在 inet_csk_listen_stop() 的 cond_resched() 处，当处于 BPF 迭代上下文（rcu_read_lock 下）时跳过该可能导致睡眠的检查，避免 invalid context BUG。
...
- kind: testing
  description: 用 BPF 程序对仍带 accept 队列子连接的 listener 调用 bpf_sock_destroy()，确认不再触发 sleeping-function-from-invalid-context BUG
