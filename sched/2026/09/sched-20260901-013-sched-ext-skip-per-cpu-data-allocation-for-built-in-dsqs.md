...
authors: [Qiurong Fang, Zhan Xusheng, Tejun Heo]
maintainers_involved: [Tejun Heo, Zhan Xusheng]
current_version: v2
patch_series:
  - version: v1
    msgid: "<20260827082329.3368255-1-fangqiurong@kylinos.cn>"
    date: '2026-08-27'
    summary: 'scx_init_dsq() 对 built-in DSQ 跳过 alloc_percpu(struct scx_dsq_pcpu)，exit_dsq() 加 !dsq->pcpu 早退；带 Fixes: 标签'
    review_outcome: 'Zhan Xusheng：浪费量级是 nr_cpu_ids^2 而非线性；Fixes: 会把纯内存优化路由到 stable，建议去掉；邮件头与 Signed-off-by 姓名不一致'
  - version: v2
    ...
    review_outcome: 'Zhan Xusheng Reviewed-by，代码正确性逐点确认；仍建议 changelog 写出总节省量而非按每 DSQ 描述'
