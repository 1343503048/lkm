...
- version: v1
  msgid: null
  date: 2026-09-06
  summary: selftests/sched_ext runner 在收到 SIGINT/SIGTERM 时设置 exit_req 而提前停止，但退出状态只反映失败测试数，导致被中断的运行在已完成测试都未失败时返回成功。改为把 exit_req 纳入失败判定，使调用方能区分「被中断」与「成功」；结果计数仍只统计实际跑过的测试。
