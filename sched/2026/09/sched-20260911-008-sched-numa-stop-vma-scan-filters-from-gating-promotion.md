...
patch_series:
  - version: v1
    ...
    date: 2026-09-05
    summary: '分层模式下允许扫描只读文件映射与 PID 不活跃 VMA（promotion-only）；sashiko 指出重扫与并发疑问，作者 09-07 自认捆绑过多并承诺 v2。'
    review_outcome: '作者自我修正：并发扫描与 mode=3 需更复杂改动，既有测试仍成立。'
  - version: v2
    msgid: '<20260911001826.2109390-1-gourry@gourry.net>'
    date: 2026-09-11
    summary: '3/4 抽 vma_is_ro_file() helper 并收敛分层判断；4/4 新增 prev_placement_scan_seq 解决并发扫描与饥饿兜底；均带 Fixes + Cc stable。'
