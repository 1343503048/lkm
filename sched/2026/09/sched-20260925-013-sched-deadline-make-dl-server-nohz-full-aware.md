...
patch_series:
  - version: v2
    ...
    review_outcome: "长期未被收取；09-24 Ionut 实测发现其满 tick 代价、给出仅 server enqueue 期间保持 tick 的变体；09-25 Juri 欢迎发 v3 并补齐 ext_server"
merge_assessment:
  likelihood: medium
  blocking_issues:
...
contribution_opportunities:
  - kind: testing
    description: "在自家 nohz_full + PREEMPT_RT 场景复现三变体对照，验证 dl_servers_stop_all hazard 与 ext_server 分支"
  - kind: review
    description: "审读变体里 inc_dl_tasks/dec_dl_tasks 对 dl_server 分支加 sched_update_tick_dependency 的锁序与幂等性"
  - kind: discussion
