...
## 背景与问题
背景见 sched-20260918-012：fair 调度类可以通过 lazy rescheduling 推迟调度边界，但 sched_ext 目前只向 BPF 调度器暴露即时抢占。本系列为 sched_ext 补上惰性抢占能力，让运行中的 sched_ext 任务 slice 到期后，把调度边界推迟到返回用户态或下一个 scheduler tick。
...
## 版本演进与当前进展
- v6（此前版本，见 sched-20260918-012）：已获 Tejun 完整 review，并提出多个命名/实现点。
