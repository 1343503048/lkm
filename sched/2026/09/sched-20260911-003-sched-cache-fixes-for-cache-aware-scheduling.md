# sched/cache: Fixes for cache aware scheduling
...
patch_series:
  - version: v1
    ...
    date: 2026-09-11
    summary: '4 补丁修复系列：1 收敛 nr_pref_llc_running 到 runnable 域（修 DELAY_DEQUEUE 下计数口径错位），2 收编 Lu Wang v4（LBF_ACTIVE_LB_LLC 保语义），3 把 sched_cache_stat 抽成 refcount 的 sched_cache_group，4 task 持引用修 UAF。基于 sched/urgent。'
    review_outcome: 'PeterZ 强烈批评 4/4 实现风格（要求函数化、指 rcu_dereference_protected(true) 错误）；Kayra 质疑 1/4 计数维护点，Tim 以 set_delayed() 顺序约束与 per-rq/per-cfs_rq 作用域差异自辩；Tim 承诺清理后发 v2。'
