# sched/fair: avoid recalculating curr status in place_entity() and requeue_delayed_entity()
...
patch_series:
  ...
  - version: v2
    ...
    review_outcome: '09-11 ping 后：Vincent 否定（可读性+无收益）；PeterZ 判定 curr==se 不可达、整补丁为 no-op，给出删除死代码的 diff；作者接受并承诺 v3。'
