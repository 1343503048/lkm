...
merge_assessment:
  ...
  - '没有任何 arch/hypervisor 侧启用者，arch specific interface 被列为合入后的工作；合入后默认 CONFIG_STEAL_GOVERNOR=m 且不加载'
  - '系列仍在变大（12→13 补丁、21→23 文件）且改的是 is_cpu_allowed() 与 sched_tick() 的 stopper push，与同期 PE/task_h_load/core-sched 改动有 rebase 冲突风险'
