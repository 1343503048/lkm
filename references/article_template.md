...
- **正文中嵌入的机读字段片段用斜体与正文区分**：`合入评估` 开头的 `likelihood=X`（或 `likelihood: X`）、`版本演进与当前进展` 开头的 `current_version: vN（…日期/msgid…）` 整段（含括号），以及行中出现的字段标签（`status=`、`blocking_issues：`、`next_action：`、`merged_branch=` 等），一律用单星号斜体包裹字段 token；字段值为受控词表取值时（如 `status=merged`）连同值一起包裹，包裹号之外的评述文字保持正常样式。例：`*likelihood=medium*。方向无异议…*blocking_issues*：宏实现有重复…*next_action*：作者改版后重新提交。`
