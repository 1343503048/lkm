...
- version: v4
  ...
  date: 2026-08-10
  summary: v4（5 patches）：Patch1 修 print_dl_rq UAF（rq->rd RCU 发布屏障+读取侧 READ_ONCE）；Patch2
    修 print_cpu UAF（rcu_dereference(rq->curr)）；Patch3 修 sched_show_numa TOCTOU（READ_ONCE(p->mm)）；Patch4
    print_cfs_stats RCU 遍历+迭代上限防 RCU stall；Patch5 per-CPU debugfs 文件。
...
- sched/core
- sched/fair
title: 'sched/fair: Use list_for_each_entry_rcu() in print_cfs_stats()'
layout: article
---
