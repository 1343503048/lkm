...
- <a class="article-ref" href="/lkm/2026/09/16/sched-20260916-002-sched-make-proxy-execution-compatible-with-sched-ext.html">sched-20260916-002</a>：Peter 继续逐 patch 评审 v13，焦点落在 08/18 的 `WF_ON_RQ` 语义（Andrea 自认「描述错了区分」）与 07/18 的 reject-DSQ 重试点设计；暂无 NAK，但 08/18 方向仍有开放讨论（Peter 抛出一个未测试的 `sched_delayed`→`is_blocked` 大重构思路）。合入可能性维持中等。
- <a class="article-ref" href="/lkm/2026/09/23/sched-20260923-002-sched-make-proxy-execution-compatible-with-sched-ext.html">sched-20260923-002</a>（今天）：Andrea Righi 发出 v14（16 枚）：基本落实 Peter 在 v13 评审中的全部意见——`WF_ON_RQ` 更名 `WF_TTUW_RQ` 并拆为预备 patch、blocked-donor admission 用 `scx_enabled()` static key 门控、retained-donor 清理移到 sched_ext ownership-transition 路径、NOHZ_FULL 下保守重开 tick。新 kselftest `enq_blocked` 实测：启用 blocked-donor admission 后 mutex wait 延迟 same-CPU 拓扑降 20.41%、cross-CPU 降 12.86%。系列方向获认可、无 NAK，进入 v14 复审等待期。

## 背景与问题
- <a class="article-ref" href="/lkm/2026/09/10/sched-20260910-002-sched-make-proxy-execution-compatible-with-sched-ext.html">sched-20260910-002</a>：代理执行把调度上下文（`rq->donor`）与执行上下文（`rq->curr`）拆开以缓解优先级反转；本系列（PATCHSET v13 sched_ext/for-7.4）目标是解除 `CONFIG_SCHED_PROXY_EXEC` 对 `!SCHED_CLASS_EXT` 的依赖，让 BPF 调度器全面接管 donor/curr 拆分后的策略与记账。
- <a class="article-ref" href="/lkm/2026/09/16/sched-20260916-002-sched-make-proxy-execution-compatible-with-sched-ext.html">sched-20260916-002</a>：该系列目标进一步明确为——让 sched_ext 感知 proxy 带来的 donor/curr 分离、blocked donor 的 runqueue 驻留、以及任务在 EXT 调度类与其它类之间切换时的清理时机。
- <a class="article-ref" href="/lkm/2026/09/23/sched-20260923-002-sched-make-proxy-execution-compatible-with-sched-ext.html">sched-20260923-002</a>（今天）：背景延续：互斥根源是 sched_ext 通过 DSQ 与 BPF 自主驱动 dispatch，而 proxy handoff 会运行一个 BPF 从未 dispatch 过的任务，导致 kfunc/helper 观测到的「当前任务」与 BPF 侧账本不一致。

## 技术方案
- <a class="article-ref" href="/lkm/2026/09/10/sched-20260910-002-sched-make-proxy-execution-compatible-with-sched-ext.html">sched-20260910-002</a>：当日无新版本代码，全部是 Peter 对既有 v13 补丁的设计反馈——03/18（NOHZ CFS bandwidth checks follow proxy donor）被质疑假设不成立（「rq->donor 是 FAIR 而 rq->curr 是 RT 不合逻辑」），04/18（避免 proxy donor 假迁移警告）被问「为何主线没触发」，08/18（WF_ON_RQ）被质疑其「already-runnable 区分」到底要表达什么，09/18（跨类转换阻塞 donor）被判定 DEQUEUE_CLASS 条件同义反复、要求把 `sched_proxy_block_task()` 收进 `switching_to_scx()`，15/18（委派 donor admission 给 BPF）给了可接受的改法。
- <a class="article-ref" href="/lkm/2026/09/16/sched-20260916-002-sched-make-proxy-execution-compatible-with-sched-ext.html">sched-20260916-002</a>：讨论落在三处具体设计——07/18（`scx_proxy_resolved()`，拟改名 `scx_proxy_reenqueue_retry()`）：sched_ext 消费远程 DSQ dispatch 来的任务时若 proxy exec 已使其成为 rq->curr/active donor 则无法迁移，需 park 到源 rq 的 reject DSQ 并在下次 proxy selection 后重试 drain；08/18（`WF_ON_RQ`）：sched_ext 需知道这次 wakeup 是否走了 `ttwu_runnable()` 而没调 activate/enqueue，Andrea 承认命名/语义不准；09/18：把 `sched_proxy_block_task()` 清理从 `sched_change_begin()` 移到 sched_ext 的 ownership-transition 路径。
- <a class="article-ref" href="/lkm/2026/09/23/sched-20260923-002-sched-make-proxy-execution-compatible-with-sched-ext.html">sched-20260923-002</a>（今天）：v14 关键设计收敛——current-task kfunc（`scx_bpf_task_running()`/`scx_bpf_cpu_curr()`/`scx_bpf_cid_curr()`）统一报告 BPF 选中的 donor 而非代为执行的 mutex owner；blocked donor 仅在 proxy resolution 成功后进入 `ops.running()`/`ops.stopping()` 会话；任何所有权切换都从干净任务状态开始（先完全 deactivate retained donor）；`scx_qmap` 加 `-X` 选项、新 kselftest `enq_blocked` 覆盖优先级反转场景。
