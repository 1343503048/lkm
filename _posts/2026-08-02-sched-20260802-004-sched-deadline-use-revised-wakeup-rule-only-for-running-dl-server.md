...
- version: v1
  ...
  date: 2026-08-02
  summary: 修正 deferred DL server 的唤醒行为：revised wakeup rule 只应用于已处于 running 状态的 dl_server，使
    deferred DL server 的唤醒真正被延迟。
  review_outcome: 已通过 tip 树 review 并进入 sched/urgent，由 Ingo Molnar 发起 pull request
    送往 Linus。
