...
- version: v4
  msgid: <unknown>
  date: 2026-08-04
  summary: active load balance 应尊重 migrate_llc_task（仅本地 LLC 内迁移）语义。延续 08-03-005：Peter
    Zijlstra 要求把 can_migrate_llc_task() 合入 task_can_migrate() 由 active LB 复用，消除两条路径语义分叉。
...
merge_assessment:
  likelihood: high
  blocking_issues:
  - 待作者按 PeterZ 的 helper 合并意见发 v5
  next_action: 等待 v5（can_migrate_llc_task() 合入 task_can_migrate()）后合入。
contribution_opportunities:
- kind: review
  description: v5 发布后审阅 can_migrate_llc_task() 合并进 task_can_migrate() 是否改变了 nohz/stop
    等其它调用点的迁移行为，回帖确认无回归（08-03-005 已提过）。
generated_at: '2026-08-05T00:25:00'
source_email_count: 1
related_articles:
- sched-20260803-005-sched-cache-honor-migrate_llc_task-semantics-in-active-load-balance
tags:
- cfs
- load_balance
- affinity
