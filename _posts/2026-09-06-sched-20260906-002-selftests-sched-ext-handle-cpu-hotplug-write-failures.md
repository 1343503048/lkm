...
- version: v1
  msgid: null
  date: 2026-09-06
  summary: selftests/sched_ext 的 toggle_online_status() 记录了 CPU 状态变更失败日志却丢弃了 write
    返回值，导致 hotplug 测试可能在一次从未触发的调度器退出上无限等待。改为返回 write 结果，在必需的 CPU 状态变更失败时同时停止两个 hotplug
    测试，并释放已获取的调度器资源、由既有 cleanup 回调重试恢复 CPU1。
...
- kind: testing
  description: 在双 vCPU VM 中用 strace 向各 CPU 状态 write 注入 EIO，确认修复后测试返回 1 且 CPU1 在线、sched_ext
    在 cleanup 后禁用
