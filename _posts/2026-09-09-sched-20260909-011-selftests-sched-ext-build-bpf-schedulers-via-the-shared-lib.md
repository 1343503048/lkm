...
merge_assessment:
  ...
  next_action: 作者补发或说明 v4 changelog；确认共享片段对 subskeleton 与编译选项默认值的处理；请 sched_ext/BPF
    侧至少给一次跨 clang 版本的构建验证
contribution_opportunities:
- kind: testing
  description: 在两个以上不同 clang 版本下构建 tools/testing/selftests/sched_ext，确认全部 skeleton/subskeleton
    生成、runner 链接且用例可运行，这类 Makefile 重构最易在作者之外的工具链组合上碎掉
- kind: review
