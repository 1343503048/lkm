...
- version: v1
  ...
  date: 2026-07-28
  summary: '15-patch series enabling proxy execution with sched_ext: split curr/donor
    references, handle proxy-exec races in DSQ transfers, delegate blocked donor admission
    to BPF schedulers, add selftest and scx_qmap support'
...
merge_assessment:
  likelihood: medium
  blocking_issues:
  - 需要 Tejun Heo 等 sched_ext maintainer review
  - 涉及 sched core 与 sched_ext 的交互，需要 Peter Zijlstra 确认
  next_action: 等待 maintainer review 反馈
contribution_opportunities:
- kind: testing
  description: 在启用 SCHED_PROXY_EXEC + SCHED_CLASS_EXT 的配置下测试 proxy execution 场景（如
    stress-ng --pipeherd），验证无 sleeping-while-atomic 警告
- kind: review
