...
- version: v3
  ...
  review_outcome: 2026-09-01 Tejun Heo Acked-by 并请 Peter Zijlstra 收取；同日 Tejun 以「同一竞态」为由让
    Andrea 的 sched_ext knob_mutex 方案让路
...
- kind: new_patch
  description: 评估该改动对已回合 sched_ext 的自有分支的侵入（改在 core 而非 ext），并清理自家可能存在的 knob_mutex
    变体
- kind: discussion
