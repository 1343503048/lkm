...
- version: v1-rcf
  ...
  date: '2026-09-07'
  summary: RFC 列出两种互斥修法：genirq 侧用 scoped_guard(cpus_read_lock) 包住 irq_thread_check_affinity()
    里的 set_cpus_allowed_ptr()（并讨论是否要求所有调用者持该锁）；或 sched 侧在 migration_cpu_stop() 中当
    __migrate_task() 未落到 dest_cpu 时把当前 CPU 补进 p->cpus_mask。改动 kernel/irq/manage.c
    +6/-2、kernel/sched/core.c +2。
...
- kind: new_patch
  description: 自家 OLK-6.6 同处缺少保护（kernel/irq/manage.c 的 set_cpus_allowed_ptr 未持 cpus_read_lock、migration_cpu_stop
    未处理 __migrate_task 失败），可先独立复现评估，待上游定路线后再决定回合形态
