...
merge_assessment:
  ...
  next_action: 等 Chen Yu 的验证结果回帖；作者可顺手处理 sched_tick_exec_ctx() 位置 nit 并收集 Tim Chen
    的 Reviewed-by 发下一版或直接等维护者排队。
contribution_opportunities:
- 在 CONFIG_SCHED_PROXY_EXEC=y + NUMA balancing 下量化 RT donor 代理执行时两个 tick hook 的缺失比例并回帖
- 把 sched_tick_exec_ctx() 移入 fair.c、恢复 task_tick_numa()/task_tick_cache() 为 static，并附
  W=1 构建结果
- 接手已被剥离的 __entity_slice_used() 代理执行核算修复（pick 时 vruntime 快照 vs task-clock 基线，含 fair-group
  reweight 口径问题）
