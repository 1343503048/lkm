...
- version: v1
  msgid: <unknown>
  date: 2026-08-19
  summary: 把 scx 仓库中领先内核副本的 tools/sched_ext/include/scx 头文件同步回内核树：__COMPAT_read_enum()
    现可在 BTF 缺 BTF_KIND_ENUM64 的内核上恢复 64 位 scx enum（新增 autogen 枚举 ABI 表）；is_migration_disabled()
    修复 v6.18+ !PREEMPT_RCU 下当前任务少报；纳入 Gavin/Changwoo 的 scx_bpf_dsq_peek() 版本门与 scx_bpf_reenqueue_local_from_anywhere()（已合
    review）；恢复 __COMPAT_scx_bpf_cpu_curr() 供 pre-v6.18 调度器，rq clock helper 文档化 idle
    CPU 的 stale-read 行为。基于 sched_ext/for-7.3-fixes (5f01293930d1)。
