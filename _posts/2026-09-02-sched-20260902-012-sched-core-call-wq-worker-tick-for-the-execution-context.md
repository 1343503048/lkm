...
- 开 PROXY_EXEC 跑 CPU 密集的 unbound workqueue，量化 wq_worker_tick() 改用 rq->curr 前后的 worker
  CPU 归属与 WORKER_CPU_INTENSIVE 判定
- '接手当天零回帖的 sched/core: fix task_sched_runtime() for proxy execution（UID 73775），它已带实测却无人回应'
- 把 task_sched_runtime / wq_worker_tick / task_tick_numa / task_tick_cache / RT watchdog
  五处 execution-context 修正整理成一个系列提交
- 确认 wq_worker_tick() 在 sched_tick() 中与 !scx_switched_all() 分支的相对位置在 sched_ext + PE
  组合下是否仍正确
