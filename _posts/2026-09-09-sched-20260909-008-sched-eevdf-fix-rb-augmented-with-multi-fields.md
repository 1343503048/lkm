...
- version: v1
  ...
  review_outcome: 09-09 04:37 K Prateek Nayak 给 Reviewed-by+Tested-by，并说明他在 2025-02
    的 EEVDF 系列中遇到过同一问题、当时在调度器层规避，认为泛化通用 rb 层更合理；另提一条 nit：min_vruntime_copy() 上方的注释未描述
    copy 语义。作者 21:20 承认是复制粘贴结果。
merge_assessment:
  likelihood: high
  blocking_issues:
...
- kind: testing
  description: 比对补丁前后 cfs_rq_min_slice()/cfs_rq_max_slice() 采样与实体 lag 分布，并向 Prateek
    追问他在 20250220093257.9380-22 处实际看到的症状是什么
- kind: new_patch
  description: '与前一日的 sched/eevdf: Fix augmented max_slice 成对回合到自家分支以恢复增广字段完整性不变式，Fixes
    引用自家分支 commit 而非上游 aef6987d8954'
