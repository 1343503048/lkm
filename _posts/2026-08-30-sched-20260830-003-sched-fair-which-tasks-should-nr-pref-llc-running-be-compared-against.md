...
- version: v1
  ...
  date: 2026-08-28
  summary: '内联补丁 sched/cache: Keep nr_pref_llc_running in the runnable domain：在 set_delayed()/clear_delayed()
    同步调整 nr_pref_llc_running，并让 account_llc_dequeue() 在 sched_delayed 仍置位时跳过减计数'
  review_outcome: 报告者 Zhan Xusheng 给出 Reviewed-by 与收支证明，确认 sd->llc_counts 保持 queued
    语义；8/30 Chen Yu 指出迁移 attach 路径仍会错误加计数，未发新 revision
...
- kind: testing
  description: 在实体多 LLC 机器上采集 nr_pref_llc_running/h_nr_runnable/nr_running，复现 delayed
    任务导致 alb_break_llc() 漏保护
- kind: extend
