...
- version: v1
  msgid: <unknown>
  date: 2026-08-03
  summary: 重排 cfs_bandwidth 结构成员，把运行期热字段（period_timer、hrtimer_active、throttled_lb
    等）聚拢，使 32 位小结构体也落在 4 字节对齐，减少内存占用。附 before/after 结构体布局。
...
merge_assessment:
  likelihood: medium
  blocking_issues:
  - 纯布局优化，需确认对热路径字段偏移无负面 cache-line 影响
  next_action: 等待 maintainer 对『聚合热字段 vs 拆分到不同 cache line』权衡的认可；属低优先级清理。
contribution_opportunities:
- kind: review
  description: cfs_bandwidth 成员聚合后热字段是否落到同一 cache line 带来正面效果，可审阅 pahole before/after
    确认无 cache-line 撕裂回归。
