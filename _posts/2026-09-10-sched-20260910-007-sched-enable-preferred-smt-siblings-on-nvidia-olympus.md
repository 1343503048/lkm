...
- version: v5
  ...
  date: '2026-09-09'
  summary: 仅删除冗余的 olympus_prefer_pe0 状态；调度器侧收口在 select_idle_sibling() 之后，arm64 侧 MIDR
    检测。
  review_outcome: 09-09 Prateek Reviewed-by+Tested-by、Peter tentatively picked up、Will
    Deacon 拒绝 MIDR 检测、Vincent 质疑 static key；09-10 Peter 回复 Will 时明确 drop 该系列，等 arm64
    侧方案重做。
merge_assessment:
  likelihood: low
  blocking_issues:
