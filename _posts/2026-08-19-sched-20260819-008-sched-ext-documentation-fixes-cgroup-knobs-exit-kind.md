...
- version: v1
  msgid: <unknown>
  date: 2026-08-19
  summary: sched-ext 文档两连修：(1) 把 cpu.max / cpu.max.burst / cpu.idle 的描述从'仅影响 fair-class'改为'也影响实现了
    cgroup_set_bandwidth / cgroup_set_idle 回调的 BPF 调度器'（与 cpu.weight 措辞对齐）；(2) 修正
    sched-ext.rst 中 ops.exit() 示例的 ei->type -> ei->kind（struct scx_exit_info 从未有 type
    字段，自引入起就是 kind），Fixes fa48e8d2c7b5。
