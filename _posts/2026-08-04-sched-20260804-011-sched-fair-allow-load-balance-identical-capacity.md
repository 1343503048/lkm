...
- version: v6
  msgid: <unknown>
  date: 2026-08-04
  summary: sched_balance_find_src_rq() 原用『~5% 额外容量』阈值避免迁到低容量 CPU，但无意中也阻止了相同容量 CPU
    间的迁移；当 CONFIG_SCHED_CLUSTER 开启时应允许跨相同容量 cluster 均衡。改用 get_actual_cpu_capacity()
    并经 sched_cluster_active 静态键保护。
  review_outcome: Christian Loehle Tested-by，Andrea Righi Tested-by，Vincent Guittot
    在 08-04 给出 review 讨论。v6 已多轮迭代。
