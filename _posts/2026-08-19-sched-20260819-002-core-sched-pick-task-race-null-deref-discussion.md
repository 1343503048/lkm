...
- version: v1
  msgid: <unknown>
  date: 2026-08-19
  summary: Peter 在 8/19 回复 Aaron Lu 7/2 报告的 core_sched pick_task() 竞态：pick_task()
    释放 core-wide 锁后未触发 RETRY_TASK 而继续，导致 rqX->core_pick 被对端置 NULL 后 NULL 解引用。Peter
    倾向在 pick 时取本地 core_task_seq 副本、末尾校验，但仍担心活锁；并点出 sched_ext 让问题更复杂（引用 8/19 另一封 sched_ext
    邮件 [1]）。
  review_outcome: 讨论中，尚无合入补丁；Peter 自述 'Only bad ideas so far'，需先理清 core-sched 与 sched_ext
    的交互。
...
merge_assessment:
  likelihood: medium
  blocking_issues:
  - 缺乏带前进进度保证的修复（RETRY_TASK 重做有活锁风险）
  - sched_ext 参与 pick 使 core-sched 的锁中断语义更复杂
