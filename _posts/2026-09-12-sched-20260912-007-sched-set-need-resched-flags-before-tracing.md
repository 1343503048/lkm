...
- version: v1
  ...
  date: 2026-09-12
  summary: set_tsk_need_resched()/__resched_curr() 在发 sched_set_need_resched_tp 前先置
    TIF，防 BPF 递归重入致栈溢出；Fixes adcc3bfa8806。
  review_outcome: Gabriele 指出与 Sechang Noh 06-27 补丁相同；Andrea 确认 exactly the same fix
    并撤回，改用 Sechang v3（2026-06-30）。
