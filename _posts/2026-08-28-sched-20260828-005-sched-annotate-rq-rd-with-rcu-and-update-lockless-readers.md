...
- version: v7
  ...
  date: '2026-08-27'
  summary: 逐点使用 rcu_dereference()/rcu_dereference_sched()/rcu_access_pointer() 保护
    rq->rd 读者
  review_outcome: Peter Zijlstra 三条反对（protected 只用于更新侧、rq->lock 非拓扑更新锁、&rq->__lock
    破坏抽象、过于啰嗦）；Vincent Guittot 指出误删注释
- version: v8
  msgid: <20260827194014.977758-2-atomlin@atomlin.com>
  date: '2026-08-28'
  summary: 引入 rcu_dereference_root_domain() 统一宏并全面替换读者
  review_outcome: 作者当天自撤：Sashiko 查出 dl_task_needs_bw_move() 仅持 cpuset_mutex 的非法解引用，virtme-ng
    复现 suspicious RCU usage
- version: v9
  msgid: <20260827221809.988394-2-atomlin@atomlin.com>
  date: '2026-08-28'
  summary: dl_task_needs_bw_move() 套 guard(rcu)()；补 syscalls.c（__sched_setscheduler/dl_task_check_affinity）与
    add_nr_running()；2/6-6/6 相对 v8 无实质变化
...
- kind: testing
  description: CONFIG_PROVE_RCU + CPU hotplug/cpuset 分区变更/DL 任务设置组合，回报 suspicious
    RCU usage
- kind: testing
