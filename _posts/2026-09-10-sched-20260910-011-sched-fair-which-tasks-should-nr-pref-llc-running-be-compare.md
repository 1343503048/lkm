...
- version: v1
  ...
  review_outcome: 09-10 Tim Chen 用 T1/T2 反例论证 h_nr_queued 分母会把睡眠任务算成 imbalance、错误打破
    T1 局部性，并指出 Lu Wang 的 ALB guard 只覆盖 migrate_llc；Chen Yu 接受并已启动 sanity 测试，结果待回报。
merge_assessment:
  likelihood: unknown
  blocking_issues:
...
- kind: testing
  description: 构造 T1（偏好本 LLC 运行）+T2（不偏好、delay-queued）场景，DELAY_DEQUEUE 开启下对比两种分母下 alb_break_llc()
    的实际决策，独立验证 Tim 的反例
- kind: new_patch
