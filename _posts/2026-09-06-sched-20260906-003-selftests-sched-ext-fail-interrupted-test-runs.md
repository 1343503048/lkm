...
- version: v1
  ...
  date: '2026-09-06'
  summary: 'selftests/sched_ext/runner.c：SIGINT/SIGTERM 置 exit_req 使 runner 提前停止，但退出码只反映
    failed 计数，被中断且已完成用例全通过时返回 0。改为 return failed > 0 || exit_req ? 1 : 0，让调用方能区分被中断与成功；结果计数仍只统计实际跑过的用例。runner.c
    单行改动（+1/-1）。'
