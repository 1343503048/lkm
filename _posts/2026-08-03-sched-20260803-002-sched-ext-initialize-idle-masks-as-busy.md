...
- version: v1
  msgid: <unknown>
  date: 2026-08-03
  summary: 内置 idle 掩码在 sched_ext 启用前把所有 online CPU 标记为 idle，导致 busy CPU 在下次 idle 转换前被错误地广播为
    idle。改为初始化为空（全部标记 busy），保守但安全；bypass 解除时每 cpu 被重调度并通过 idle-to-idle re-pick 填充真实
    idle 掩码。
...
- kind: testing
  description: 在 sched_ext 启用后、各 cpu 首次 idle 转换前的窗口内验证内置 idle 掩码不再把 busy cpu 暴露给调度器，可补一个
    selftest 断言。
...
- sched_ext
- idle
title: 'sched_ext: Initialize idle masks as busy'
layout: article
---
