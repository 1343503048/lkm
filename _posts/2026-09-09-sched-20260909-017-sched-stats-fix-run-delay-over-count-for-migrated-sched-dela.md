...
- kind: testing
  description: 构造长睡眠的 delayed 任务并在睡眠期用 migrate_swap() 触发迁移，比对 /proc/<pid>/sched 的
    run_delay 与真实等待时间，为补丁补上目前完全缺失的实测证据
- kind: review
...
- kind: new_patch
  description: 回合过 delayed dequeue 的分支（如 OLK 6.6）均受影响，适合直接回合这枚单行修复，Fixes 需指向自家分支引入该特性的
    commit 而非上游 152e11f6df29
