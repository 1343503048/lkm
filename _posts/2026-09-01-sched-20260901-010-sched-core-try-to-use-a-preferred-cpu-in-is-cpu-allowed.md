...
- version: v11
  msgid: null
  date: null
  summary: steal_governor / preferred CPU 系列的 05/12：is_cpu_allowed()/select_fallback_rq()
    在受限时优先挑选仍被允许的 preferred CPU，含 task_can_sched_on_preferred() 与架构专属 valid_mask 求交分支（背景见
    sched-20260831-007）
  review_outcome: 09-01 三封回帖只争论启用范围：Yury Norov 认为 arm64 缺测试；Vincent Guittot 反对按架构设门槛（cpumask
    本就复用）；Dietmar Eggemann 指出不开 allow_mismatched_32bit_el0 时 arm64 与已测架构无差异，且 task_cpu_possible_mask()
    已在 core.c 与 cpuset.c 中被使用，故该特殊分支在主流 arm64 配置下不可达
...
- kind: testing
  description: 在 arm64 服务器上分别以 allow_mismatched_32bit_el0 开/关跑 steal_governor + preferred
    CPU，验证 Dietmar 的"行为无差异"推断
- kind: testing
