...
- version: v1
  ...
  date: 2026-08-16
  summary: 4 patch 修复 sched_ext core-sched 任务排序：倒置的 ops.core_sched_before 调用、用 runnable_at
    统一等待追踪、跨调度器层级排序、删除死测试。
...
- kind: review
  description: 可复核 scx_qmap 之外的第三方 BPF 调度器是否因 ops.core_sched_before 语义反转需同步翻转比较（作者提示仅
    scx_qmap 已知受影响）。
...
- sched_ext
- core_sched
title: 'sched_ext: Drop the dead SCX_DEQ_CORE_SCHED_EXEC test in dequeue_task_scx()'
layout: article
---
