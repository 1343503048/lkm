...
- version: v3
  ...
  date: 2026-08-14
  summary: v3（4/4）：sched_ext 自带约 130 行 libbpf+bpftool+vmlinux.h+BPF object+skeleton
    构建逻辑，替换为包含共享的 tools/testing/selftests/lib.bpf.mk，使 sched_ext 成为继 cgroup、hid 之后第三个使用该片段的树内消费者。28
    个 skeleton/sub-skeleton 公开 API 前后一致。
  review_outcome: v3 发出，Suggested-by Eduard Zingerman 与 Mykola Lysenko。Makefile 减
    116 行。
