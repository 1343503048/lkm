...
- version: v1
  ...
  summary: 把 RB_DECLARE_CALLBACKS 泛化为 RB_DECLARE_CALLBACKS_MULTI（由调用方提供 RBCOPY），原单字段宏改为自动生成
    _copy_single 后转调 MULTI；fair.c 新增 min_vruntime_copy() 同时搬运 min_vruntime/min_slice/max_slice，并改用
    MULTI 声明 min_vruntime_cb。
...
- kind: testing
  description: 用不同 slice/权重实体高频入队出队（混合 sched_latency 与 cgroup 份额动态调整）比对补丁前后 cfs_rq_min_slice()/cfs_rq_max_slice()
    采样与实体 lag 分布，补上缺失的症状证据
- kind: review
