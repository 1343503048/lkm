...
- version: v2
  ...
  date: 2026-07-29
  summary: 6 patch：QoS notifier 订阅 + per-CPU resume QoS 失效 + 聚合值按 CPU 缓存 + 2 个 selftest；v1
    改动点未在本日缓存邮件中体现
...
- kind: testing
  description: 在 x86/arm64 机器上跑系列自带的两个 cpuidle selftest，并用 ftrace function_graph 复测
    menu_select 中 cpuidle_governor_latency_req 占比，把数据回帖
- kind: review
  description: 审查 QoS notifier 失效路径是否覆盖所有约束更新入口（全局 CPU latency、wakeup latency、per-CPU
    resume latency），有无缓存过期窗口
...
- idle
- perf
title: 'selftests/cpuidle: add idle-state disable selftest'
layout: article
---
