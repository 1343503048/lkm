---
layout: default
tag: "cfs"
title: "标签: cfs"
article_count: 42
---

- [sched-20260824-006](/lkm/2026/08/24/sched-20260824-006-sched-fair-null-deref-v4.19.html) `bug/critical/under_review` — sched/fair: NULL deref in pick_next_task_fair (v4.19)
- [sched-20260824-009](/lkm/2026/08/24/sched-20260824-009-sched-flatten-the-pick.html) `discussion/high/under_review` — sched: Flatten the pick
- [sched-20260821-003](/lkm/2026/08/21/sched-20260821-003-sched-flatten-the-pick-v3-benchmark.html) `discussion/none/under_review` — sched: Flatten the pick
- [sched-20260806-002](/lkm/2026/08/06/sched-20260806-002-sched-fair-nohz-fully-idle-core-v5.html) `feature/none/under_review` — sched/fair: Prefer fully idle cores for NOHZ balancing
- [sched-20260806-008](/lkm/2026/08/06/sched-20260806-008-sched-cache-honor-migrate_llc_task-active-lb.html) `fix/medium/under_review` — sched/cache: honor migrate_llc_task semantics in active load balance
- [sched-20260806-009](/lkm/2026/08/06/sched-20260806-009-sched-fair-sync-wakeup-target-waker-core.html) `feature/none/under_review` — sched/fair: Let sync wakeups target the waker's core
- [sched-20260806-012](/lkm/2026/08/06/sched-20260806-012-fuse-wakeup-hints-to-scheduler.html) `feature/none/draft` — fuse: give wakeup hints to the scheduler for synchronous requests
- [sched-20260806-013](/lkm/2026/08/06/sched-20260806-013-sched-fair-load-balance-identical-capacity-v6.html) `feature/none/under_review` — sched/fair: Allow load balancing between CPUs of identical capacity
- [sched-20260806-014](/lkm/2026/08/06/sched-20260806-014-sched-numa-hygon-remote-socket-distance-v2.html) `fix/medium/under_review` — sched/numa: Apply remote socket distance averaging for Hygon CPUs
- [sched-20260805-002](/lkm/2026/08/05/sched-20260805-002-sched-fair-prefer-fully-idle-cores-nohz-v3-v4.html) `feature/none/under_review` — sched/fair: Prefer fully idle cores for NOHZ balancing
- [sched-20260805-004](/lkm/2026/08/05/sched-20260805-004-sched-fair-remove-dead-throttled-check-pick-task-fair.html) `cleanup/low/superseded` — sched/fair: Remove dead throttled check in pick_task_fair()
- [sched-20260805-005](/lkm/2026/08/05/sched-20260805-005-sched-cache-honor-migrate_llc_task-active-lb.html) `fix/medium/under_review` — sched/cache: honor migrate_llc_task semantics in active load balance
- [sched-20260805-006](/lkm/2026/08/05/sched-20260805-006-sched-fair-sync-wakeup-target-waker-core.html) `feature/none/under_review` — sched/fair: Let sync wakeups target the waker's core
- [sched-20260805-007](/lkm/2026/08/05/sched-20260805-007-sched-fair-wf_sync-semantics-wake-affine-doc.html) `feature/none/under_review` — sched/fair: Let sync wakeups target the waker's core
- [sched-20260805-008](/lkm/2026/08/05/sched-20260805-008-sched-fair-decline-wf_sync-stacking-when-waker-llc-busier.html) `feature/none/under_review` — sched/fair: decline WF_SYNC stacking when waker LLC is the busier share
- [sched-20260804-005](/lkm/2026/08/04/sched-20260804-005-sched-fair-prefer-fully-idle-cores-for-nohz-balancing.html) `feature/none/under_review` — sched/fair: Allow load balancing between CPUs of identical capacity
- [sched-20260804-006](/lkm/2026/08/04/sched-20260804-006-sched-fair-sync-wakeups-target-waker-core.html) `discussion/none/under_review` — sched/fair: Let sync wakeups target the waker's core
- [sched-20260804-007](/lkm/2026/08/04/sched-20260804-007-sched-cache-honor-migrate_llc_task-active-lb-v2.html) `fix/medium/under_review` — sched/fair: Allow load balancing between CPUs of identical capacity
- [sched-20260804-011](/lkm/2026/08/04/sched-20260804-011-sched-fair-allow-load-balance-identical-capacity.html) `feature/none/under_review` — sched/fair: Allow load balancing between CPUs of identical capacity
- [sched-20260803-004](/lkm/2026/08/03/sched-20260803-004-sched-fair-prefer-waker-cpu-for-non-smt-reciprocal-sync-wakeups.html) `discussion/none/under_review` — sched/fair: Prefer waker CPU for non-SMT reciprocal sync wakeups
- [sched-20260803-005](/lkm/2026/08/03/sched-20260803-005-sched-cache-honor-migrate_llc_task-semantics-in-active-load-balance.html) `fix/medium/under_review` — sched/cache: honor migrate_llc_task semantics in active load balance
- [sched-20260803-008](/lkm/2026/08/03/sched-20260803-008-sched-adjust-cfs_bandwidth-layout-to-save-memory.html) `fix/low/under_review` — sched_ext: Fixes for v7.2-rc6
- [sched-20260801-003](/lkm/2026/08/01/sched-20260801-003-sched-fair-sync-wakeups-target-wakers-core.html) `feature/none/under_review` — sched/fair: Prefer waker CPU for non-SMT reciprocal sync wakeups
- [sched-20260801-004](/lkm/2026/08/01/sched-20260801-004-sched-fair-prefer-waker-cpu-non-smt-reciprocal-sync-wakeups-v3.html) `feature/none/under_review` — sched/fair: Let sync wakeups target the waker's core
- [sched-20260801-005](/lkm/2026/08/01/sched-20260801-005-sched-fair-prefer-fully-idle-cores-for-nohz-balancing-v3.html) `feature/none/under_review` — sched/fair: Prefer fully idle cores for NOHZ balancing
- [sched-20260801-006](/lkm/2026/08/01/sched-20260801-006-sched-cache-honor-migrate-llc-task-in-active-load-balance.html) `fix/medium/under_review` — sched/cache: honor migrate_llc_task semantics in active load balance
- [sched-20260731-002](/lkm/2026/07/31/sched-20260731-002-sched-fair-skip-numa-balancing-scan-on-memoryless-nodes.html) `bug/high/under_review` — sched/fair: Skip NUMA balancing scan on memoryless nodes
- [sched-20260731-005](/lkm/2026/07/31/sched-20260731-005-fuse-give-wakeup-hints-to-scheduler-for-sync-requests.html) `feature/none/rfc` — [RFC PATCH] fuse: give wakeup hints to the scheduler for synchronous requests
- [sched-20260731-006](/lkm/2026/07/31/sched-20260731-006-sched-cache-task-cache-work-v9.html) `discussion/medium/under_review` — -- DO NOT APPLY!!! -- sched/cache/debug: Add trace event and sched feature to track scan cost
- [sched-20260731-007](/lkm/2026/07/31/sched-20260731-007-sched-fair-prefer-fully-idle-cores-nohz-v2-incremental.html) `feature/none/under_review` — sched/fair: Prefer fully idle cores for NOHZ balancing
- [sched-20260731-008](/lkm/2026/07/31/sched-20260731-008-sched-reject-policy-changes-with-sched-flag-keep-params-v2.html) `fix/medium/under_review` — sched: Reject policy changes with SCHED_FLAG_KEEP_PARAMS
- [sched-20260730-002](/lkm/2026/07/30/sched-20260730-002-sched-fair-cgroup-mode-default-netperf-regression.html) `bug/high/under_review` — [linux-next:master] [sched/fair]  fb1050ac8e: netperf.Throughput_Mbps 14.6% regression
- [sched-20260730-004](/lkm/2026/07/30/sched-20260730-004-sched-fair-prefer-waker-cpu-non-smt-reciprocal-sync-wakeups.html) `feature/none/under_review` — sched/fair: Prefer waker CPU for non-SMT reciprocal sync wakeups
- [sched-20260730-008](/lkm/2026/07/30/sched-20260730-008-sched-fair-prefer-fully-idle-cores-nohz-balancing-v2.html) `feature/none/under_review` — sched/fair: Prefer fully idle cores for NOHZ balancing
- [sched-20260730-010](/lkm/2026/07/30/sched-20260730-010-sched-cache-task-cache-work-epoch-race.html) `discussion/medium/under_review` — : [PATCH v8 1/2] sched/cache: Reduce the overhead of task_cache_work by only scan the visisted cpus
- [sched-20260729-004](/lkm/2026/07/29/sched-20260729-004-sched-core-skip-rq-avg-idle-update-without-a-valid-idle-stam.html) `fix/medium/under_review` — sched/core: Skip rq->avg_idle update without a valid idle_stamp
- [sched-20260729-005](/lkm/2026/07/29/sched-20260729-005-sched-cache-reduce-the-overhead-of-task-cache-work-by-only-s.html) `feature/none/under_review` — : [PATCH v8 1/2] sched/cache: Reduce the overhead of task_cache_work by only scan the visisted cpus
- [sched-20260728-003](/lkm/2026/07/28/sched-20260728-003-sched-fair-prefer-waker-cpu-for-non-smt-reciprocal-sync-wake.html) `feature/none/under_review` — sched/fair: Prefer waker CPU for non-SMT reciprocal sync wakeups
- [sched-20260728-005](/lkm/2026/07/28/sched-20260728-005-sched-cache-reduce-the-overhead-of-task-cache-work-by-only-s.html) `discussion/none/under_review` — : [PATCH v8 1/2] sched/cache: Reduce the overhead of task_cache_work by only scan the visisted cpus
- [sched-20260728-006](/lkm/2026/07/28/sched-20260728-006-sched-cache-fix-a-thread-aggregation-conflict-when-there-is.html) `fix/medium/under_review` — sched/cache: Fix a thread aggregation conflict when there is one runnable task
- [sched-20260728-010](/lkm/2026/07/28/sched-20260728-010-sched-idle-sysbench-threads-regression-after-f4c31b07b136.html) `bug/high/under_review` — sched idle sysbench threads regression after f4c31b07b136
- [sched-20260726-001](/lkm/2026/07/26/sched-20260726-001-sched-make-proxy-execution-compatible-with-sched-ext.html) `feature/none/under_review` — sched/core: Avoid false migration warning for proxy donors
